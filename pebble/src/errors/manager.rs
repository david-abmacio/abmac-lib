...
verdict::display_error! {
    ...
    pub enum PebbleManagerError<Id: core::fmt::Debug, E: core::fmt::Debug + core::fmt::Display> {
        #[display("storage error: {source}")]
        Storage { source: StorageError },

        #[display("DAG error: {source}")]
        DAG { source: DAGError },

        #[display("checkpoint never added: {state_id:?}")]
        NeverAdded { state_id: Id },

        #[display("storage load failed for checkpoint {state_id:?}: {reason}")]
        StorageLoadFailed { state_id: Id, reason: String },

        #[display("dependency {dep_id:?} missing during rebuild of {for_id:?}")]
        DependencyMissing { dep_id: Id, for_id: Id },

        #[display("serialization failed for checkpoint {state_id:?}: {source}")]
        Serialization { state_id: Id, source: E },

        #[display("deserialization failed for checkpoint {state_id:?}: {source}")]
        Deserialization { state_id: Id, source: E },

        #[display("flush failed: {source}")]
        FlushFailed { source: E },

        #[display("rebuild failed for state {state_id:?}: {reason}")]
        RebuildFailed { state_id: Id, reason: String },

        #[display("cannot rebuild checkpoint {state_id:?}: dependency width {width} exceeds hot_capacity {limit}")]
        DependencyWidthExceeded { state_id: Id, width: usize, limit: usize },

        #[display("checkpoint size {size} exceeds maximum {max}")]
        CheckpointTooLarge { size: usize, max: usize },

        #[display("internal inconsistency: {detail}")]
